...
# z_s value array
z_s=[ complex( -4.64935,7.14205), complex(-6.70391,3.48532) ,complex(-7.29348,0), complex(-6.70391 ,-3.48532),complex(-4.64935,-7.14205)]
#value of M
M=length(z_s)
...
Psi_f[:,1]=((2*a/pi)^0.25).*exp.(-a*(x.- sqrt(2)*alpha).^2 +complex(0,b).*x)
...
gif(anim, "Harmonic wavefunction.gif", fps = 30)
##########################################################################################################
#error analysis
...
Psi_theory= Psi_f[:,2]
error_x= real.(Psi_f[:,n_tsigma]).-real.(Psi_theory)
